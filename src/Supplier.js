...
function Supplier() {
  ...
  const filteredData = apiData.filter((supplier) => {
    return (
      ...
      supplier.address
        .toLowerCase()
        .includes(searchTermAddress.toLowerCase()) &&
      supplier.phonel &&
  ...
  const handleRowSubmit = () => {
    ...
    if (rowSelected && selectedItem) {
      ...
      axios
        .put(`${apiUrl}/supplier/${selectedItem.supplierId}`, formData, {
        ...
        .then((response) => {
          ...
          setApiData((prevData) =>
            prevData.map((item) =>
              item.supplierId === selectedItem.supplierId ? response.data : item
            )
  ...
  const handleDelete = (id) => {
    ...
    axios
      .delete(`${apiUrl}/supplier/${id}`, { withCredentials: true })
      .then((response) => {
        ...
        setApiData((prevData) =>
          prevData.filter((row) => row.supplierId !== id)
        );
      })
      .catch((error) => {
        if (error.response && error.response.status === 500) {
          ...
          toast.error(
            "Failed to delete supplier because it is mentioned somewhere: " +
              error.message
          );
        } else {
