...
function Return() {
  ...
  const [
    searchTermSentForRaisingTicketOn,
    setSeaarchTermSentForRaisingTicketOn,
  ] = useState("");
  const [searchTermSentForTicketOn, setSeaarchTermSentForTicketOn] =
    useState("");
  ...
  const filteredData = apiData.filter((supplier) => {
    return (
      supplier.date
        .toString()
        .toLowerCase()
        .includes(searchTermDate.toLowerCase()) && // Convert to string
      supplier.skucode
        .toString()
        .toLowerCase()
      ...
      supplier.trackingNumber
        .toString()
        .toLowerCase()
        .includes(searchTermTrackingNumber.toLowerCase()) && // Convert to string
      supplier.returnCode
        .toString()
        .toLowerCase()
        .includes(searchTermReturnCode.toLowerCase()) &&
      supplier.okStock
        .toString()
        .toLowerCase()
        .includes(searchTermOkStock.toLowerCase()) &&
      supplier.sentForRaisingTicketOn
        .toString()
        .toLowerCase()
        .includes(searchTermSentForRaisingTicketOn.toLowerCase()) &&
      supplier.sentForTicketOn
        .toString()
        .toLowerCase()
        .includes(searchTermSentForTicketOn.toLowerCase()) &&
      (!searchTermLocation ||
        (supplier.location &&
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const ipmResponse = await axios.get(
        `${apiUrl}/itemportalmapping/${ipmId}`,
        {
          params: { email: user.email },
          withCredentials: true,
        }
      ...
      const response = await axios.post(`${apiUrl}/return`, formData, {
        withCredentials: true,
      });
    ...
    if (rowSelected && selectedItem) {
      ...
      axios
        ...
        .then((response) => {
          ...
          setApiData((prevData) =>
            prevData.map((item) =>
              item.returnId === selectedItem.returnId ? response.data : item
            )
        ...
        .catch((error) => {
          ...
          toast.error(
            "Failed to update Return: " + error.response.data.message
          );
        });
    }
  };
  ...
  const postData = (data) => {
    axios
      .post(`${apiUrl}/return`, data, { withCredentials: true })
      .then((response) => {
  ...
  const handleDelete = (id) => {
    ...
    axios
      .delete(`${apiUrl}/return/${id}`, { withCredentials: true })
      .then((response) => {
              ...
              <Form.Group as={Col} md="4" controlId="validationCustom01">
                ...
                <Form.Select
                  ...
                  onChange={(e) => {
                    ...
                    const selectedIpm = ipmList.find(
                      (sku) => sku.portal === selectedPortal
                    );
                    if (selectedIpm) {
            ...
            <Table striped bordered hover>
                  ...
                  <th>
                    ...
                    <span style={{ margin: "0 10px" }}>
                      <input
                        ...
                        onChange={(e) =>
                          setSeaarchTermReturnCode(e.target.value)
                        }
                      />
                    </span>
                  </th>
                  <th>
                    ...
                    <span style={{ margin: "0 10px" }}>
                      <input
                        ...
                        onChange={(e) =>
                          setSeaarchTermTrackingNumber(e.target.value)
                        }
                      />
                    </span>
                  </th>
                  ...
                  <th>
                    ...
                    <span style={{ margin: "0 10px" }}>
                      <input
                        ...
                        onChange={(e) =>
                          setSeaarchTermSentForRaisingTicketOn(e.target.value)
                        }
                      />
                    </span>
                  </th>
                  <th>
                    ...
                    <span style={{ margin: "0 10px" }}>
                      <input
                        ...
                        onChange={(e) =>
                          setSeaarchTermSentForTicketOn(e.target.value)
                        }
                      />
                    </span>
                  </th>
              ...
              <tbody>
                {filteredData.map((returns) => (
                  <tr
                    key={returns.returnId}
                    onClick={() => handleRowClick(returns)}
                  >
