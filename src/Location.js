...
function LocationForm() {
  ...
  const filteredData = apiData.filter(location => {
    return (
      (location.locationName && location.locationName.toLowerCase().includes(searchTermLocationName.toLowerCase()))
    );
  });
  ...
  const handleSubmit = (event) => {
    ...
    if (rowSelected && selectedItem) {
      ...
      axios.post(`${apiUrl}/api/locations`, formData)
        ...
        .catch((error) => toast.error(`Failed to add location: ${error.message}`));
    }
  };
    ...
    if (rowSelected && selectedItem) {
      ...
      axios.put(`${apiUrl}/api/locations/${selectedItem.locationId}`, formData)
        .then(response => {
          ...
          setApiData(prevData => prevData.map(item => item.locationId === selectedItem.locationId ? response.data : item)); // Update the specific item
  ...
  const handleDelete = (id) => {
    axios.delete(`${apiUrl}/api/locations/${id}`)
      ...
      .catch(error => toast.error(`Failed to delete location: ${error.message}`));
  };
          ...
          <Table striped bordered hover>
            <thead>
              <tr>
                <th></th>
                <th>
                <SwapVertIcon style = {{cursor: 'pointer', marginRight: "2%"}}variant="link" onClick={() => requestSort('locationName')}>
                  </SwapVertIcon>
                  Location Name
                  ...
                  <span style={{ margin: '0 10px' }}>
                    <input
                      ...
                      onChange={(e) => setSearchTermLocationName(e.target.value)}
                    />
                  </span>
                </th>
              </tr>
