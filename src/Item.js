...
const handleRefresh = () => {
  ...
  const uniqueUnit = [...new Set(apiData.filter(item => item.unit !== null).map(item => item.unit))];
  ...
  const handleRowSubmit = () => {
    if (rowSelected && selectedItem) {
      ...
      axios
        .put(`http://localhost:8080/item/supplier/${selectedItem.itemId}`, formData)
        .then((response) => {
...
const postData = async (data) => {
  ...
  const downloadTemplate = () => {
    const templateData = [
        { skucode: '', description: '', packOf: '', parentSKU: '', group1: '', group2: '', group3: '', sizeRange: '', size: '', unit: '', barcode: '', sellingPrice: '', mrp: '', sellerSKUCode: '', img: '', supplierName: '' , phone: ''} 
    ];
    ...
    saveAs(new Blob([s2ab(wbout)], { type: 'application/octet-stream' }), 'itemTemplate.xlsx');
  };
