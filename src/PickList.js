...
const PicklistComponent = () => {
  ...
  const filteredOrders = orders.filter((order) => {
    return (
      (filters.date === "" ||
        new Date(order.date).toLocaleDateString().includes(filters.date)) &&
      (filters.orderNo === "" || order.orderNo.includes(filters.orderNo)) &&
      (filters.portalOrderNo === "" ||
        order.portalOrderNo.includes(filters.portalOrderNo)) &&
      (filters.portal === "" || order.portal.includes(filters.portal)) &&
      (filters.sellerSKU === "" ||
        order.items[0].sellerSKUCode.includes(filters.sellerSKU)) &&
      (filters.skuCode === "" ||
        order.items[0].skucode.includes(filters.skuCode)) &&
      (filters.orderQty === "" ||
        order.qty.toString().includes(filters.orderQty)) &&
      (filters.bomCode === "" ||
        (bomCodes[order.orderNo] || "").includes(filters.bomCode))
    );
  });
  ...
  useEffect(() => {
    const fetchBomsForOrder = async (orderNo) => {
      try {
        ...
        const bomCodeResponse = await axios.get(
          `${apiUrl}/picklists/bom/default/bomCode?orderNo=${orderNo}`,
          { params: { email: user.email }, withCredentials: true }
        );
        ...
        console.log(
          `Initial BOM for order ${orderNo} = ${bomCodeResponse.data}`
        );
      } catch (error) {
  ...
  useEffect(() => {
    axios
      .get(`${apiUrl}/picklists/orderData`, {
  ...
  useEffect(() => {
    axios
      .get(`${apiUrl}/picklists/merged/picklist`, {
        params: { email: user.email },
        withCredentials: true,
      })
      .then((response) => {
    ...
    if (orderNo === undefined) {
      ...
      if (event.target.checked) {
        const promises = orders.map((order) =>
          axios
            .get(
              `${apiUrl}/picklists/getSelectedOrderData?orderNo=${
                order.orderNo
              }&bomCode=${bomCodes[order.orderNo]}`,
              { params: { email: user.email }, withCredentials: true }
            )
            .then((response) => {
            ...
            .catch((error) => {
              console.error(
                `Error fetching data for orderNo ${order.orderNo}:`,
                error
              );
        ...
        Promise.all(promises)
          .then((results) => {
            const orderData = results.reduce(
              (acc, data) => [...acc, ...data],
              []
            );
      ...
      if (event.target.checked) {
        try {
          const orderDataResponse = await axios.get(
            `${apiUrl}/picklists/getSelectedOrderData?orderNo=${orderNo}&bomCode=${bomCode}`,
            { params: { email: user.email }, withCredentials: true }
          );
        ...
        setSelectedOrderData((prevData) =>
          prevData.filter((order) => order.orderNo !== orderNo)
        );
      }
    }
  };
  ...
  const generatePicklist = () => {
    ...
    axios
      .get(`${apiUrl}/picklists/user/email`, {
        params: { email: user.email },
        withCredentials: true,
      })
      .then((response) => {
  ...
  const generatePicklistWithNumber = async (pickListNumber) => {
    ...
    try {
      ...
      await Promise.all([
        ...
        axios.post(
          `${apiUrl}/picklists/not/generated/orders`,
          {
            locationId: location.locationId,
            locationName: location.locationName, // optional
            userEmail: user.email,
          },
          {
            withCredentials: true,
        ...
        .then(([picklistResponse, ordersResponse]) => {
          ...
          // console.log("at line 336:" + ordersResponse.data);
  ...
  const selected = (selectedOrders) => {
    return selectedOrders.reduce((acc, picklist) => {
      ...
      const existingGroup = acc.find(
        (group) => group.sellerSKU === picklist.sellerSKU
      );
      if (existingGroup) {
  ...
  const handleDelete = (pickListNumber) => {
    ...
    axios
      .delete(`${apiUrl}/picklistdata/picklistnumber/${pickListNumber}`, {
        params: { email: user.email },
        withCredentials: true,
      })
      .then((response) => {
        ...
        setPicklistData((prevData) =>
          prevData.filter((row) => row.pickListNumber !== pickListNumber)
        );
        axios
          .post(
            `${apiUrl}/picklists/not/generated/orders`,
          ...
          .then((response) => {
            // console.log("at line 429:" + response.data);
  ...
  const handleRowClick = (event, orderNo) => {
    ...
    if (!isChecked) {
      axios
        .get(`${apiUrl}/picklists/getSelectedOrderData?orderNo=${orderNo}`, {
          params: { email: user.email },
          withCredentials: true,
        })
        .then((response) => {
  ...
  const handleDownload1 = async (pickListNumber) => {
    try {
      const picklistItems = picklistData.filter(
        (picklist) => picklist.pickListNumber === pickListNumber
      );

      if (picklistItems.length === 0) {
        console.error(
          `Picklist with pickListNumber ${pickListNumber} not found.`
        );
        return;
      }
      ...
      const logoURL =
        "https://media.licdn.com/dms/image/D560BAQF6CchqkqZEEQ/company-logo_200_200/0/1704887637105/techjyot___india_logo?e=2147483647&v=beta&t=S1jLov5GABl39n8XPksGcm8GIQsmvMTLl84RwYZNL-8"; // Replace with your actual base64 or URL
      pdf.addImage(logoURL, "PNG", 10, 10, 30, 15); // Adjust the position and size as needed

      // Add heading
      pdf.setFontSize(18);
      pdf.setFont("helvetica", "bold");
      pdf.text("PickList", pdf.internal.pageSize.getWidth() / 2, 20, {
        align: "center",
      });
  ...
  const handleLocationChange = (selectedLocation) => {
    ...
    axios
      .post(`${apiUrl}/picklists/not/generated/orders`, selectedLocation, {
        params: { email: user.email },
        withCredentials: true,
      })
      .then((response) => {
        setOrders(response.data);
        console.log("Orders fetched successfully at line 566:", response.data);
      })
      .catch((error) => {
        console.log("Error getting orders: " + error);
      });
            ...
            <div className="locationSelect">
              <Form.Select
                ...
                onChange={(e) => {
                  ...
                  const selectedLocation = locations.find(
                    (loc) => loc.locationId === parseInt(selectedLocationId)
                  );
              ...
              >
                ...
                {locations &&
                  locations.map(
                    (
                      loc // Ensure locations is defined
                    ) => (
                ...
                {currentItems1.map((order) => (
                  <tr key={uuidv4()}>
                    ...
                    <td>
                      ...
                      >
                        <option value="">
                          {bomCodes[order.orderNo] || "Select Bom Code"}
                        </option>
      ...
      <Accordion>
        ...
        <AccordionDetails>
            ...
            <Table striped bordered hover>
                ...
                <tr>
                  ...
                  <th>
                    <SwapVertIcon
                      ...
                      variant="link"
                      onClick={() => requestSort("pickListNumber")}
                    ></SwapVertIcon>
                    Pick List Number
                  </th>
                  <th>
              ...
              <tbody>
                {picklistData.map((picklist, index) => {
                  ...
                  const isFirstRowForPickListNumber =
                    index ===
                    picklistData.findIndex(
                      (p) => p.pickListNumber === picklist.pickListNumber
                    );

                  const deleteButtonCell = isFirstRowForPickListNumber ? (
                    <td
                      rowSpan={rowsForPickListNumber}
                  ...
                  const downloadButtonCell = isFirstRowForPickListNumber ? (
                    <td rowSpan={rowsForPickListNumber}>
                      <button
                        onClick={() => handleDownload1(picklist.pickListNumber)}
                      >
                        Download
                      </button>
                    </td>
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td>
                        {picklist.rackNumber ? picklist.rackNumber : "N/A"}
                      </td>
                    </tr>
                  );
                })}
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <Pagination>
              {Array.from({
                length: Math.ceil(picklistData.length / itemsPerPage),
              }).map((_, index) => (
                <Pagination.Item
                  key={index}
