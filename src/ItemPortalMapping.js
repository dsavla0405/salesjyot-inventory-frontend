...
function ItemPortalMapping() {
    ...
    if (sortConfig.key) {
      const aValue =
        sortConfig.key === "supplier"
          ? a[sortConfig.key]?.supplierName
          : a[sortConfig.key];
      const bValue =
        sortConfig.key === "supplier"
          ? b[sortConfig.key]?.supplierName
          : b[sortConfig.key];
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .get(`${apiUrl}/item/supplier/search/${supplierId}/${skucode}`, {
      ...
      .then((response) => {
        if (response.data) {
          ...
          axios
            .post(`${apiUrl}/itemportalmapping`, formData, {
              withCredentials: true,
            })
            .then((response) => {
            ...
            .catch((error) => {
              ...
              toast.error(
                "Failed to add item portal mapping: " +
  ...
  const downloadTemplate = () => {
    ...
    saveAs(
      ...
      "itemPortalMappingTemplate.xlsx"
    );
  };
  ...
  const fetchSupplierAndItem = (supplierName, skucode) => {
    ...
    return axios
      .get(`${apiUrl}/supplier/name/${supplierName}`, {
        params: { email: user.email },
        withCredentials: true,
      })
      .then((supplierResponse) => {
        ...
        return axios
          .get(
            `${apiUrl}/item/supplier/search/${supplier.supplierId}/${skucode}`,
  ...
  const handleSupplierChange = (event, name) => {
    if (name) {
      ...
      if (selectedSupplier) {
        ...
        axios
          .get(
            `${apiUrl}/item/supplier/search/seller/${selectedSupplier.supplierName}`,
            { params: { email: user.email }, withCredentials: true }
          )
          .then((response) => {
  ...
  const handleDelete = (id) => {
    ...
    axios
      .delete(`${apiUrl}/itemportalmapping/${id}`, { withCredentials: true })
      .then((response) => {
      ...
      .catch((error) => {
        ...
        toast.error(
          "Failed to delete item portal mapping: " + error.response.data.message
        );
      });
  ...
  const handleRowSubmit = () => {
    ...
    if (rowSelected && selectedItem) {
      ...
      axios
        .put(`${apiUrl}/itemportalmapping/${selectedItem.id}`, formData, {
          withCredentials: true,
        })
        .then((response) => {
        ...
        .catch((error) => {
          ...
          toast.error(
            "Failed to update item portal mapping: " +
              error.response.data.message
          );
        });
    }
  ...
  const postData = (data) => {
    axios
      .post(`${apiUrl}/itemportalmapping`, data, { withCredentials: true })
      .then((response) => {
  ...
  const uniquePortals = [
    ...new Set(
      apiData.filter((item) => item.portal !== null).map((item) => item.portal)
    ),
  ];
          ...
          <Form noValidate validated={validated} onSubmit={handleSubmit}>
            <Row className="mb-3">
              ...
              <Form.Group as={Col} md="4" controlId="validationCustom02">
                <Form.Label>
                  Seller/Supplier <span style={{ color: "red" }}>*</span>
                </Form.Label>
                <Form.Select
                  required
                  onChange={(e) =>
            ...
            <Row className="mb-3">
              <Form.Group as={Col} md="4" controlId="validationCustom01">
                <Form.Label>
                  Portal SKUcode <span style={{ color: "red" }}>*</span>
                </Form.Label>
                <Form.Control
                  required
                  type="text"
                ...
                <tr>
                  ...
                  <th>
                    <SwapVertIcon
                      style={{ cursor: "pointer", marginRight: "2%" }}
                      variant="link"
                      onClick={() => requestSort("portalSkuCode")}
                    ></SwapVertIcon>
                    Portal SKUCode
                    <span style={{ margin: "0 10px" }}>
                      <input
                  ...
                  <th>
                    <SwapVertIcon
                      style={{ cursor: "pointer", marginRight: "2%" }}
                      variant="link"
                      onClick={() => requestSort("sellerSkuCode")}
                    ></SwapVertIcon>
                    SKUCode
                    <span style={{ margin: "0 10px" }}>
                      <input
