...
function StockCount() {
    ...
    return data.filter(
      (item) =>
        ...
        (item.item?.sellerSKUCode
          ?.toLowerCase()
          .includes(filters.sellerSKUCode.toLowerCase()) ||
          !item.item) &&
        // Check count
        item.count
          .toString()
          .toLowerCase()
          .includes(filters.count.toLowerCase()) &&
        // Check combo's qtyToMake (or skip if no combo)
        (item.combo?.qtyToMake
          ?.toString()
          .toLowerCase()
          .includes(filters.qtyToMake.toLowerCase()) ||
          !item.combo)
    );
  };
  ...
  const createChartData = (data, type = "item") => ({
    labels: data.map((item) =>
      type === "item"
        ? item.item?.skucode
        : item.combo?.comboName?.skucode || "Unknown"
    ),
    datasets: [
      {
        label: "Stock Count",
      ...
      <Row>
        ...
        <Col md={6}>
          ...
          <Card className="mt-4">
            <Card.Body>
              <Bar
                data={createChartData(itemData, "item")}
                options={{
                  plugins: {
                    title: {
                      display: true,
                      text: "Item Stock Counts",
                    },
                  },
                }}
              />
        ...
        <Col md={6}>
          ...
          <Card className="mt-4">
            <Card.Body>
              <Bar
                data={createChartData(comboData, "combo")}
                options={{
                  plugins: {
                    title: {
                      display: true,
                      text: "Combo Stock Counts",
                    },
                  },
                }}
              />
