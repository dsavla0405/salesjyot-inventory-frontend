...
function LocationForm() {
  ...
  useEffect(() => {
    axios
      ...
      }) // Replace with your items API endpoint
      .then((response) => {
        // console.log("running from 78........");
  ...
  const handleItemSelect = (item, isSelected) => {
    if (isSelected) {
      ...
      // shubham
      axios
        .get(`${apiUrl}/storage/${item.itemId}`, {
          withCredentials: true,
        })
        .then((response) => {
          setStorageMap((prev) => ({
            ...prev,
            [item.itemId]: response.data,
          }));
        })
      ...
      setSelectedItems((prevItems) =>
        prevItems.filter((selectedItem) => selectedItem.itemId !== item.itemId)
      );
    }
    setStorageMap((prev) => {
      const updated = { ...prev };
  ...
  const filteredData = apiData.filter((stockTransfer) => {
    return (
      stockTransfer.toLocation.locationName &&
      stockTransfer.toLocation.locationName
        .toLowerCase()
        .includes(searchTermToLocation.toLowerCase()) &&
      stockTransfer.fromLocation.locationName &&
      stockTransfer.fromLocation.locationName
        .toLowerCase()
        .includes(searchTermFromLocation.toLowerCase())
    );
  });
  ...
  useEffect(() => {
    axios
      .get(`${apiUrl}/api/stocktransfer/user/email`, {
        params: { email: user.email },
        withCredentials: true,
      })
      .then((response) => {
        setApiData(response.data);
        console.log("running from 171........");
      })
  ...
  const handleSubmit = async (event) => {
    ...
    for (const item of selectedItems) {
      if (item.itemId && item.quantity > 0) {
        // Ensure both item and quantity are valid
        try {
          const allStorages = Object.values(storageMap).flat();
          const storageUsed = allStorages.find(
            (storage) => storage.storageId === item.storageId
          );
          const formData = {
            ...
            fromStorage: storageUsed,
          };
          console.log("formData::", JSON.stringify(formData));
          // Make the API call to post the data
          await axios
            .post(`${apiUrl}/api/stocktransfer/create`, formData, {
              withCredentials: true,
            })
            .then((response) => {
          ...
          toast.success(
            `Item ${item.skucode} with quantity ${item.quantity} posted successfully`
          );
        } catch (error) {
          toast.error(
            `Failed to post item ${item.skucode}: ${error.response.data}`
          );
        }
      } else {
  ...
  const handleRowSubmit = () => {
    ...
    if (rowSelected && selectedItem) {
      ...
      axios
        .put(
          `${apiUrl}/api/stocktransfer/update/${selectedItem.stockTransferId}`,
          formData,
          { withCredentials: true }
        )
        .then((response) => {
          ...
          setApiData((prevData) =>
            prevData.map((item) =>
              item.stockTransferId === selectedItem.stockTransferId
                ? response.data
                : item
            )
        ...
        .catch((error) => {
          ...
          toast.error(
            "Failed to update Stock Transfer: " + error.response.data.message
          );
        });
    }
  };
  ...
  const handleDelete = (id) => {
    axios
      .delete(`${apiUrl}/api/stocktransfer/delete/${id}`, {
        withCredentials: true,
      })
      .then(() => {
        toast.success("Stock Transfer deleted successfully");
        setApiData((prevData) =>
          prevData.filter((location) => location.stockTransferId !== id)
        );
      })
      .catch((error) =>
        toast.error(`Failed to delete Stock Transfer: ${error.message}`)
      );
  };
  ...
  const postData = (data) => {
    axios
      .post(`${apiUrl}/api/stocktransfer/create`, data, {
        withCredentials: true,
      })
      .then((response) => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        console.log(
          "API response for locations at line 360-----: ",
          response.data
                    ...
                    <tr key={item.itemId}>
                      ...
                      <td>
                        <Form.Select
                          ...
                          disabled={!storageMap[item.itemId]}
                          onChange={(e) => {
                            const selectedRack = e.target.value;
                            setSelectedItems((prevItems) =>
                              prevItems.map((i) =>
                                i.itemId === item.itemId
                                  ? {
                                      ...
                                      binNumber: "", // Reset bin when rack changes
                                      storageId: null,
                                    }
                                  : i
                              )
                        ...
                        >
                          ...
                          {[
                            ...new Set(
                              storageMap[item.itemId]?.map(
                                (loc) => loc.rackNumber
                              )
                            ),
                          ].map((rack, idx) => (
                            <option key={idx} value={rack}>
                              {rack}
                            </option>
                          ))}
                        </Form.Select>
                      ...
                      <td>
                        <Form.Select
                          ...
                          onChange={(e) => {
                            ...
                            const selectedRack = selectedItems.find(
                              (i) => i.itemId === item.itemId
                            )?.rackNumber;

                            const storageEntry = storageMap[item.itemId]?.find(
                              (loc) =>
                                loc.rackNumber === selectedRack &&
                                loc.binNumber === selectedBin
                            );
                            console.log(
                              "hiiiiiiii::" + JSON.stringify(storageEntry)
                            );
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            const bins = storageMap[item.itemId]?.filter(
                              (loc) => loc.rackNumber === selectedRack
                            );
                            return bins?.map((bin, idx) => (
                              <option key={idx} value={bin.binNumber}>
                                {bin.binNumber}
                              </option>
                            ));
                          })()}
                ...
                <th>
                  ...
                  <span style={{ margin: "0 10px" }}>
                    <input
                      ...
                      onChange={(e) =>
                        setSearchTermFromLocation(e.target.value)
                      }
                    />
                  </span>
                </th>
